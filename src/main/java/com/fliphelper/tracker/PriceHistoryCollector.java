...
public class PriceHistoryCollector {
    ...
    private static final int MAX_TRACKED_ITEMS = 2000; // Cap total items to prevent memory leak
    ...
    public void recordSnapshot() {
        ...
        if (priceHistory.size() > MAX_TRACKED_ITEMS) {
            ...
            for (int i = 0; i < toRemove && i < keysToEvict.size(); i++) {
                ...
                seededItems.remove(evictId);
            }
            log.debug("Evicted {} items from price history (cap: {})", toRemove, MAX_TRACKED_ITEMS);
        }
    ...
    public void seedTopItems(int count) {
        try {
            ...
            for (PriceAggregate agg : topItems) {
                ...
                // Rate limit: 200ms between requests (runs on background executor, not client thread)
                try {
