...
public class DumpDetector
{
    ...
    private static final double PRICE_CHANGE_THRESHOLD = 5.0;  // 5% deviation = alert
    private static final double VOLUME_SPIKE_MULTIPLIER = 3.0;  // 3x normal volume = spike
    private static final double SPREAD_WARNING_PERCENT = 10.0;  // 10% spread = unusual
    ...
    public List<PriceAlert> detectAnomalies()
    {
        ...
        for (PriceAggregate agg : priceService.getAggregatedPrices().values())
        {
            ...
            // Determine if this is alertworthy
            ...
            if (isDump || isPump || isVolSpike || isWideSPread)
            {
                ...
                if (isDump) {
                    ...
                    alert.setSeverity(Math.abs(Math.min(highDeviation, lowDeviation)));
                } else if (isPump) {
                    alert.setType(AlertType.PUMP);
                    alert.setSeverity(Math.max(highDeviation, lowDeviation));
                } else if (isVolSpike) {
