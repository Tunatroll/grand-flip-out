...
public interface GrandFlipOutConfig extends Config
{
    ...
    @ConfigItem(
        keyName = "marginCheckHotkey",
        name = "Margin Check Price",
        description = "Copy the instant-buy price for a margin check to clipboard. Buy 1 item at this price, then sell 1 at the instant-sell price to discover the true margin.",
        section = hotkeysSection,
        position = 9
    )
    default Keybind marginCheckHotkey()
    {
    ...
    @ConfigSection(
        ...
        description = "Connect to a locally-running market analysis engine for anomaly detection, price predictions, and market insights",
        position = 9
    )
    String neuralNetSection = "neuralnet";

    @ConfigItem(
        keyName = "enableNeuralNet",
        name = "Enable Neural Net",
        description = "Connect to the market analysis engine on localhost for real-time anomaly detection and market insights. The engine must be running separately.",
        section = neuralNetSection,
        position = 0
    )
    default boolean enableNeuralNet()
    {
        return false;
    }

    @ConfigItem(
        keyName = "neuralNetUrl",
        name = "Neural Net URL",
        description = "Base URL of the market analysis engine API (default: http://localhost:5000)",
        section = neuralNetSection,
        position = 1
    )
    default String neuralNetUrl()
    {
    ...
    @ConfigItem(
        ...
        name = "Margin Assist +/- %",
        description = "Percent delta used for clipboard buy/sell assist variants.",
        section = hotkeysSection,
        position = 6
    )
    @Range(min = 1, max = 25)
    ...
    @ConfigItem(
        ...
        description = "Copy buy price baseline and +/- margin variants for active item.",
        section = hotkeysSection,
        position = 7
    )
    ...
    @ConfigItem(
        ...
        description = "Copy sell price baseline and +/- margin variants for active item.",
        section = hotkeysSection,
        position = 8
    )
    ...
    @ConfigItem(
        ...
        description = "Copy a full buy/sell/qty assist block for the first active item.",
        section = hotkeysSection,
        position = 9
    )
